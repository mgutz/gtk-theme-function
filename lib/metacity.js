...
var BUTTON_STYLE_DEFAULT = 0; // display a fg image on top of bg image offset down 1
...
function blend(color, blendColor, blendFactor) {
    ...
    if (_.isArray(blendFactor)) {
        ...
        for (var i = 0; i < blendFactor.length; i++) {
...
var ioTheme = {
    ...
    topBars: {
        windowFocused: {
            ...
            // [0,1]=top 50% [2,3]=bottom 50%
            gradients: shade(colors.bgNormal, [0.89, 0.75, 0.72, 0.56]),

            // white alpha: [0]=top [1]=left [2]=right [3]=bottom
            highlights: [0.04, 0.00, 0.00, 0.00],

            // white alpha
            highlightArc: shade(colors.bgNormal, 0.83)
        },
        windowUnfocused: {
            name: 'bg_topbar_unfocused',
            gradients: shade(colors.bgNormal, [1.17, 1.02, 0.98, 0.83]),
            highlights: [0.05, 0.0, 0.0, 0.0],
            highlightArc: shade(colors.bgNormal, 1.11)
        }
    },
    ...
    titles: {
        focused: {
            ...
            text: shade(colors.bgNormal, 1.7), 
            style: styles.titleTopInset,
            highlight: shade(colors.bgNormal, 0.4) 
        },

        unfocused: {
            text: shade(colors.bgNormal, 0.5),
    ...
    outlines: { 
        windowFocused: { 
            ...
            outline: {
                ...
                gradients: shade(colors.bgNormal, [0.7, 0.5, 0.5, 0.5]),
                arc: shade(colors.bgNormal, 0.73)
            }
        },
        windowUnfocused: {
            topBar: {
                shadow: shade(colors.bgNormal, 0.70)
            },
            outline: {
                // [0]=title top [1]=client top [2]=client bottom
                gradients: shade(colors.bgNormal, [0.7, 0.5, 0.5, 0.5]),
    ...
    buttons: {
        common: {
            ...
            colors: shade(colors.bgNormal, [1.40, 1.60, 1.20,     0.6, 0.40, 0.3]),
            bgColors: shade(colors.bgNormal, [0.55, 0.55, 0.55,   1.1, 1.1, 1.1]),
            bgYOffset: [-1, 1]
        },

        close: {
            colors: [shade(colors.bgNormal, 1.4), shade(DEBIAN_RED, 1.0), shade(DEBIAN_RED, 0.8)].concat(
                     shade(colors.bgNormal, [0.60, 0.40, 0.3])),
            bgColors: shade(colors.bgNormal, [0.55, 0.55, 0.55,   1.1, 1.1, 1.1]),
            bgYOffset: [-1, 1]
        }
    }
...
var soxIoTheme = merger.cloneextend(ioTheme, {
    buttonPadding: 0,
    titleBarLRPadding: 8,
...
var soxIoThemeLeft = merger.cloneextend(soxIoTheme, {
    buttons: {
        utility_unshade: {
...
function run() {
    ...
    var themes = {
        ...
        "SO X. IO": [soxIoTheme, rounded],
        "SO X. IO Left": [soxIoThemeLeft, rounded],
